...
export default function SiteVisitReports() {
  const { data: visits = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("visit_compliance_summary" as any)
  ...
  const funnel = useMemo(() => {
    const total = (dd as any[]).length;
    const converted = (dd as any[]).filter((x) => x.outcome_code === "converted").length;
    const followUp = (dd as any[]).filter((x) => x.outcome_code === "follow_up").length;
    const declined = (dd as any[]).filter((x) => x.outcome_code === "declined").length;
    return [
