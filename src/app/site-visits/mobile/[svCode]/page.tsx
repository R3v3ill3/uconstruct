...
export default function SiteVisitMobile() {
  const { svCode = "" } = useParams<{ svCode: string }>() || { svCode: "" };
  ...
  const saveWHS = async () => {
    if (!visit) return;
    const { error } = await supabase.from("whs_assessment").upsert({ site_visit_id: visit.id, rating_code: whsRating });
  ...
  const startVisit = async () => {
    if (!visit) return;
    const { error } = await supabase.from("site_visit").update({ status_code: "in_progress", started_at: new Date().toISOString() }).eq("id", visit.id);
    if (error) return toast.error((error as any).message);
  ...
  const completeVisit = async () => {
    if (!visit) return;
    const { error } = await supabase.from("site_visit").update({ status_code: "completed", completed_at: new Date().toISOString(), outcomes_locked: true }).eq("id", visit.id);
    if (error) return toast.error((error as any).message);
