...
export function Checklist({ svId, onChanged }: { svId: string; onChanged?: (map: Record<string,string>) => void }) {
  ...
  useEffect(() => {
    (async () => {
      const [{ data: its }, { data: existing }] = await Promise.all([
        supabase.from("checklist_item" as any).select("code,label").eq("is_active", true),
        supabase.from("site_visit_checklist_response").select("checklist_item_code,response_option_code").eq("site_visit_id", svId),
      ]);
      setItems(its || []);
  ...
  async function saveResponse(itemCode: string, option: string) {
    ...
    setResponses(next);
    await supabase.from("site_visit_checklist_response").upsert({ site_visit_id: svId, checklist_item_code: itemCode, response_option_code: option });
    onChanged?.(next);
  }
