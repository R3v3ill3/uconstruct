...
-- 1.2 Extend profiles table with activity status
ALTER TABLE public.profiles 
ADD COLUMN is_active BOOLEAN NOT NULL DEFAULT true,
ADD COLUMN last_login_at TIMESTAMP WITH TIME ZONE;

-- Handle the existing role column properly
-- First remove the default
ALTER TABLE public.profiles ALTER COLUMN role DROP DEFAULT;

-- Then update the column type with explicit casting
ALTER TABLE public.profiles 
ALTER COLUMN role TYPE app_role USING 
  CASE 
    WHEN role::text = 'admin' THEN 'admin'::app_role
    WHEN role::text = 'organiser' THEN 'organiser'::app_role
    WHEN role::text = 'viewer' THEN 'viewer'::app_role
    ELSE 'viewer'::app_role
  END;

-- Set new default
ALTER TABLE public.profiles ALTER COLUMN role SET DEFAULT 'viewer'::app_role;
