-- Retry migration without non-immutable predicates in indexes
...
-- 3) Indexes (no CURRENT_DATE in predicates)
...
-- Optional: allow only one open-ended head contractor per project at a time
CREATE UNIQUE INDEX IF NOT EXISTS uniq_open_head_contractor_per_project
  ON public.project_employer_roles (project_id)
  WHERE role = 'head_contractor'::public.project_role AND end_date IS NULL;
