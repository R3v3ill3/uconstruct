-- Add estimated_worker_count to employers table
ALTER TABLE public.employers 
ADD COLUMN estimated_worker_count integer DEFAULT 0;
...
-- Create indexes for efficient aggregate queries
CREATE INDEX IF NOT EXISTS idx_worker_placements_employer_worker ON public.worker_placements(employer_id, worker_id);
CREATE INDEX IF NOT EXISTS idx_worker_placements_employer_site ON public.worker_placements(employer_id, job_site_id);
CREATE INDEX IF NOT EXISTS idx_workers_union_status ON public.workers(union_membership_status);

-- Create a view for employer analytics to make queries more efficient
