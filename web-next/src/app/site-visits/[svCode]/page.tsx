...
export default function SiteVisitDetail() {
  ...
  const params = useParams<{ svCode: string }>();
  const svCode = params?.svCode || "";
  ...
  useEffect(() => {
    if (!visit) return;
    setObjective((visit as any).objective || "");
    setEstimatedWorkers((visit as any).estimated_workers_count ?? "");
    setScheduledAt((visit as any).scheduled_at ? new Date((visit as any).scheduled_at).toISOString().slice(0, 16) : "");
  }, [visit]);
  ...
  const startVisit = async () => {
    if (!visit) return;
    setIsStarting(true);
    const { error } = await supabase.from("site_visit").update({ status_code: "in_progress", started_at: new Date().toISOString() }).eq("id", (visit as any).id);
    setIsStarting(false);
    if (error) return toast.error((error as any).message);
  ...
  useEffect(() => {
    const { data: d } = loadDraft<{ objective: string; estimatedWorkers: number | "" }>(DraftKey(svCode as string, "prep"));
    if (d) {
  ...
  useEffect(() => {
    saveDraft(DraftKey(svCode as string, "prep"), { objective, estimatedWorkers });
  ...
  const saveWHS = async () => {
    ...
    const { data: assess, error: aErr } = await supabase
      .from("whs_assessment")
      .upsert({ site_visit_id: (visit as any).id, rating_code: whsRating })
      .select("id")
      .single();
    if (aErr) return toast.error((aErr as any).message);
    for (const b of breaches) {
      if (!b.title) continue;
      const { error: bErr } = await supabase
        .from("whs_breach")
        .insert({ whs_assessment_id: (assess as any).id, title: b.title, notes: b.notes || null, rating_code: whsRating });
      if (bErr) return toast.error((bErr as any).message);
    }
  ...
  const saveEntitlements = async () => {
    if (!visit) return;
    const { error } = await supabase.from("entitlements_audit").upsert({ site_visit_id: (visit as any).id, ...ent }).select("id").single();
    if (error) return toast.error((error as any).message);
  ...
  const { data: delegateRoleTypes = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("delegate_role_type" as any).select("code,label");
      if (error) throw error;
      return data || [];
    },
  });
  ...
  const saveDelegate = async () => {
    ...
    const { data: d, error } = await supabase
      ...
      .upsert({ site_visit_id: (visit as any).id, present: delegatePresent })
      .select("id")
      .single();
    if (error) return toast.error((error as any).message);
    for (const role_code of Object.keys(roleRatingsState)) {
      ...
      await supabase.from("delegate_role_rating").upsert({ delegate_assessment_id: (d as any).id, ...rr });
    }
  ...
  const saveDD = async () => {
    ...
    const { error } = await supabase.from("dd_conversion_attempt").insert(payload);
    if (error) return toast.error((error as any).message);
    if (ddOutcome === "converted") {
      const shouldUpdate = confirm("Mark worker membership as Direct Debit active now?");
      if (shouldUpdate) {
        await supabase.from("worker_memberships").upsert({ worker_id: ddWorkerId, payment_method: "direct_debit", dd_status: "active" as any });
      }
    }
  ...
  return (
    <div className="p-6">
      <div className="mb-4 text-sm text-muted-foreground">
        <span className="mr-3">Status: <strong>{(visit as any).status_code || "draft_prep"}</strong></span>
        {(visit as any).scheduled_at && <span className="mr-3">Scheduled: {new Date((visit as any).scheduled_at).toLocaleString()}</span>}
        {(visit as any).started_at && <span className="mr-3">Started: {new Date((visit as any).started_at).toLocaleString()}</span>}
        {(visit as any).completed_at && <span>Completed: {new Date((visit as any).completed_at).toLocaleString()}</span>}
      </div>
        ...
        <TabsContent value="prep">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <Checklist svId={(visit as any).id} onChanged={(map) => setChecklistMap(map)} />
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <EbaQuickAttach siteVisitId={(visit as any).id} />
              </div>
              <div className="flex flex-col gap-2 md:flex-row md:items-center md:gap-3">
                <div className="flex gap-3">
                  <Button onClick={savePrep} disabled={!!(visit as any).outcomes_locked}>Save</Button>
                  <Button variant="outline" onClick={startVisit} disabled={!!(visit as any).outcomes_locked || (visit as any).status_code === "in_progress" || (visit as any).status_code === "completed" || isStarting || !requiredChecklistOk}>Start visit</Button>
                  <Button variant="outline" onClick={completeVisit} disabled={!!(visit as any).outcomes_locked || (visit as any).status_code !== "in_progress" || isCompleting}>Complete visit</Button>
                </div>
                {!requiredChecklistOk && (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                <Button onClick={saveWHS} disabled={!!(visit as any).outcomes_locked}>Save WHS</Button>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                ...
                <Button onClick={saveEntitlements} disabled={!!(visit as any).outcomes_locked}>Save Entitlements</Button>
              </CardContent>
            </Card>
                ...
                <div className="space-y-2">
                  ...
                  {(delegateRoleTypes as any[]).map((r) => (
                    <div key={r.code} className="grid grid-cols-2 gap-2 items-center">
                      ...
                      <select className="border rounded h-10 w-full px-2" value={roleRatingsState[r.code] || "3"} onChange={(e)=> setRoleRatingsState((prev)=> ({ ...prev, [r.code]: e.target.value }))}>
                        {["1","2","3","4","5"].map((v)=>(<option key={v} value={v}>{v}</option>))}
                      </select>
                    </div>
                  ))}
                </div>
                <Button onClick={saveDelegate} disabled={!!(visit as any).outcomes_locked}>Save Delegate</Button>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                <WorkerQuickPicker employerId={(visit as any).employer_id} value={ddWorkerId || ""} onChange={(id) => setDdWorkerId(id)} />
                <div>
                  <Label>Outcome</Label>
                ...
                <Button onClick={saveDD} disabled={!ddWorkerId || !!(visit as any).outcomes_locked}>Save Attempt</Button>
              </CardContent>
            </Card>
          </div>
        </TabsContent>
