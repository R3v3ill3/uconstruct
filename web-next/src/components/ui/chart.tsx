      ...
      <div ref={ref} className={cn("grid min-w-[8rem] items-start gap-1.5 rounded-lg border border-border/50 bg-background px-2.5 py-1.5 text-xs shadow-xl", className)}>
        ...
        <div className="grid gap-1.5">
          {(payload as any[]).map((item, index) => {
            ...
            return (
              <div key={(item as any).dataKey} className={cn("flex w-full flex-wrap items-stretch gap-2 [&>svg]:h-2.5 [&>svg]:w-2.5 [&>svg]:text-muted-foreground", indicator === "dot" && "items-center")}
              >
                {formatter && (item as any)?.value !== undefined && (item as any).name ? (
                  ...
                  <>
                    ...
                    >
                      <div className="grid gap-1.5">{nestLabel ? tooltipLabel : null}<span className="text-muted-foreground">{itemConfig?.label || (item as any).name}</span></div>
                      {(item as any).value && <span className="font-mono font-medium tabular-nums text-foreground">{(item as any).value.toLocaleString()}</span>}
                    </div>
                  </>
                )}
              </div>
    ...
    return (
      <div ref={ref} className={cn("flex items-center justify-center gap-4", verticalAlign === "top" ? "pb-3" : "pt-3", className)}>
        {(payload as any[]).map((item) => {
          ...
          const itemConfig = getPayloadConfigFromPayload(config, item, key);
          return (
            <div key={(item as any).value} className={cn("flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-muted-foreground")}
            >
